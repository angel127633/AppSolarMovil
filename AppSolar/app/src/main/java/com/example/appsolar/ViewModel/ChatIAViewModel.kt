...
class ChatIAViewModel : ViewModel() {
    ...
    fun sendMessage(text: String) {
        ...
        // 1. REGISTRAR EL MENSAJE DEL USUARIO EN LA INTERFAZ
        messages.add(
            Message(
                text = text,
                isUser = true,
        ...
        // 2. EVALUAR SI CONTIENE CONTEXTO ENERGÉTICO VALIDO (OBLIGA A IR A LA API)
        val containsEnergyContext = "aire" in lower || "a/c" in lower || "acondicionado" in lower ||
                "panel" in lower || "solar" in lower || "paneles" in lower || "mejorar" in lower ||
                "optimizar" in lower || "rendimiento" in lower || "instalar" in lower ||
                "poner" in lower || "comprar" in lower || "lavanderia" in lower || "lavandería" in lower ||
                "lavado" in lower || "factura" in lower || "recibo" in lower || "energia" in lower ||
                "energía" in lower || "alta" in lower || "cara" in lower || "mucho" in lower ||
                "subio" in lower || "ahorro" in lower || "ahorrar" in lower || "consumo" in lower ||
                "costo" in lower || "kwh" in lower || "luz" in lower || "red" in lower

        // 3. DETECTAR CORTESÍA PURA (SALUDOS / DESPEDIDAS / AGRADECIMIENTOS)
        val isGreeting = "hola" in lower || "buenas" in lower || "hello" in lower || "hi" in lower ||
                "buenos dias" in lower || "buen día" in lower || "buen dia" in lower ||
                "buenas tardes" in lower || "buenas noches" in lower || "quien eres" in lower ||
                "quién eres" in lower || "que eres" in lower || "qué eres" in lower || "ayuda" in lower ||
                "puedes ayudarme" in lower || "que puedes hacer" in lower || "qué puedes hacer" in lower

        val isFarewell = "adiós" in lower || "adios" in lower || "chao" in lower || "hasta luego" in lower ||
                "nos vemos" in lower || "bye" in lower || "gracias" in lower || "muchas gracias" in lower

        val isPureCourtesy = (isGreeting || isFarewell)

        // 4. TOMAR DECISIÓN DE RUTA: LOCAL O API

        // CASO A: Es un saludo, despedida o cortesía pura sin contenido de energía -> Respuesta Local Inmediata
        if (isPureCourtesy && !containsEnergyContext) {
            val localReply = generateFallbackResponse(text)
            messages.add(
                Message(
                    text = localReply,
                    isUser = false,
            ...
            return // Bloquea la llamada a la API y la barra de carga
        }

        // CASO B: No contiene términos energéticos y tampoco es cortesía -> Fuera del manual de entrenamiento
        if (!containsEnergyContext) {
            messages.add(
                Message(
                    text = "Lo siento ⚠️, esa pregunta parece estar fuera de mi área de especialización. Eso no está en mi manual de entrenamiento.\n\nActualmente estoy enfocado de forma estricta en la optimización energética, ahorro eléctrico y sistemas de energía solar para tu negocio.",
                    isUser = false,
                    animated = false
                )
            )
        ...
        // CASO C: SI TIENE CONTENIDO DE ENERGÍA -> ENVIAR A LA API EXTERNA
    ...
    private fun generateFallbackResponse(question: String): String {
        ...
        return when {
            "aire" in lower || "a/c" in lower || "aire acondicionado" in lower -> {
                listOf(
            ...
            ("mejorar" in lower || "optimizar" in lower || "rendimiento" in lower) &&
                    ("panel" in lower || "solar" in lower || "paneles" in lower) -> {
                listOf(
            ...
            ("instalar" in lower || "poner" in lower || "comprar" in lower) &&
                    ("panel" in lower || "solar" in lower || "paneles" in lower) -> {
                listOf(
            ...
            "lavanderia" in lower || "lavandería" in lower || "lavado" in lower -> {
                listOf(
                    ...
                    "Las áreas de lavandería consumen bastante energía debido a procesos continuos de lavado y secado 🌤️. Una buena práctica energética consiste en programar los ciclos más pesados durante el mediodía solar y dejar procesos ligeros para horas de menor generación.\n\nTambién ayuda bastante realizar mantenimiento frequent en secadoras y motores, ya que equipos en mal estado suelen trabajar con mayor esfuerzo y desperdiciar energía innecesariamente. Optimizar estos procesos puede representar un ahorro importante a largo plazo.",
            ...
            ("factura" in lower || "recibo" in lower || "energia" in lower) &&
                    ("alta" in lower || "cara" in lower || "mucho" in lower || "subio" in lower) -> {
                listOf(
            ...
            "ahorro" in lower || "ahorrar" in lower || "factura" in lower || "consumo" in lower -> {
                listOf(
                    "Existen varias estrategias que pueden ayudarte a reducir significativamente el consumo energético ⚡ sin afectar demasiado la operación diaria del negocio. Una de las más efectivas es desplazar cargas pesadas hacia las horas de mayor radiación solar, donde el aprovechamiento energético es mucho más eficiente.\n\nTambién es recomendable monitorear equipos de alto consumo como aire acondicionado, refrigeración y lavandería, ya que normalmente representan gran parte del gasto eléctrico mensual. Pequeñas optimizaciones en horarios and uso operativo pueden traducirse en ahorros importantes a mediano plazo.",
                    ...
                    "Una buena gestión energética combina tecnología, planificación y hábitos de consumo eficientes 🌤️. Aprovechar correctamente la radiación solar disponible durante el día permite disminuir la dependencia de la red y optimizar el rendimiento energético general.\n\nAdemás, identificar los equipos que más consumen ayuda a tomar decisiones más inteligentes sobre horarios de operación and posibles mejoras futuras."
                ).random()
            }

            // =========================================
            // CAPTURA LOCAL DE SALUDOS ENTRANTES
            // =========================================
            "hola" in lower || "holaa" in lower || "buenas" in lower || "hello" in lower || "hi" in lower -> {
                listOf(
                    "¡Hola! 👋 Soy tu asistente energético inteligente ⚡. ¿En qué te puedo colaborar hoy con respecto a tu consumo o paneles solares?",
                    "¡Hola! ☀️ Estoy aquí para ayudarte a optimizar el consumo energético de tu negocio y aprovechar mejor la energía solar disponible.",
                    "¡Bienvenido! ⚡ Puedes preguntarme sobre paneles solares, ahorro energético, facturas eléctricas y estrategias de climatización."
                ).random()
            }

            "buenos dias" in lower || "buen día" in lower || "buen dia" in lower -> {
                listOf(
                    "¡Buenos días! ☀️ Espero que tengas una excelente jornada. ¿Deseas revisar alguna estrategia de ahorro para hoy?",
            ...
            // =========================================
            // CAPTURA LOCAL DE DESPEDIDAS Y CORTESÍA
            // =========================================
            "gracias" in lower || "muchas gracias" in lower -> {
                listOf(
                    "¡Con gusto! ⚡ Estoy aquí para ayudarte a optimizar el consumo energético y aprovechar mejor la energía solar.",
                    "¡Siempre a la orden! ☀️ Si tienes más preguntas sobre el sistema de energía solar, aquí estaré.",
                    "¡Me alegra ayudarte! 🌤️ Recuerda que pequeñas optimizaciones energéticas pueden generar grandes ahorros."
                ).random()
            }

            "adiós" in lower || "adios" in lower || "chao" in lower || "hasta luego" in lower || "nos vemos" in lower || "bye" in lower -> {
                listOf(
                    "¡Hasta luego! 👋 Que tengas un excelente día optimizando tu energía.",
                    "¡Nos vemos! ☀️ Recuerda mantener tus consumos pesados en las horas de sol.",
                    "¡Chao! ⚡ Quedo atento a la próxima vez que necesites asistencia energética."
                ).random()
            }

            "quien eres" in lower || "quién eres" in lower || "que eres" in lower || "qué eres" in lower -> {
                listOf(
            ...
            "ayuda" in lower || "puedes ayudarme" in lower || "que puedes hacer" in lower || "qué puedes hacer" in lower -> {
                listOf(
            ...
            // CUALQUIER OTRA CONSULTA INTERNA QUE NO HAYA SIDO FILTRADA CORRECTAMENTE POR LAS CONDICIONES ANTERIORES
            else -> {
                "Lo siento ⚠️, esa pregunta parece estar fuera de mi área de especialización. Eso no está en mi manual de entrenamiento.\n\nPor favor, hazme una consulta relacionada de forma estricta con paneles solares, climatización, consumos o facturas de energía."
            }
        }
    }
}